...
public class Player1Controller : MonoBehaviour
{
    ...
    public void Initialise()
    {
        ...
        gridPosition_snake_xy = new Vector2Int(4, 24);
    ...
    private void MoveSnake()
    {
        ...
        // collision with the player 1 wall
        ...
        for (int i = 0; i < Player2Controller.player2Controller.playerBodyPositionList_segments.Count; i++)
        {
            int sx = Player2Controller.player2Controller.playerBodyPositionList_segments[i].x;
            int sy = Player2Controller.player2Controller.playerBodyPositionList_segments[i].y;
    ...
    {
        GameController.gameController.UpdatePlayer2Score(1);
    ...
    private void UpdateSnakeHead(int i, Vector2Int currentBodySegment)
    {
        if (playerBodySize > 1)
        {
            ...
            {
                sprite = GameAssets.gameAsset.player1HeadDownSprite;
            }

            // head right
            else if (currentBodySegment.x > nseg.x)
            {
                sprite = GameAssets.gameAsset.player1HeadRightSprite;
            }

            // head up
            else if (currentBodySegment.y > nseg.y)
            {
                sprite = GameAssets.gameAsset.player1HeadUpSprite;
            }

            // head left
            else if (currentBodySegment.x < nseg.x)
            {
                sprite = GameAssets.gameAsset.player1HeadLeftSprite;
            }
        }
    ...
    {
        GameObject snakeBodyGameObject = Player1ObjectPooler.player1ObjectPooler.GetPooledObject();
